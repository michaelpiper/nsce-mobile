    ...
    Navigator.of(context)
      ...
      return Scaffold(
        ...
        body: SingleChildScrollView(
          child: Html(padding: EdgeInsets.all(12.0), data: payload),
        ),
      );
    }));
    return Container();
...
//      call notification and initialize firebase only when in release mode
  ...
  void dispose() {
    super.dispose();
    if (_updateme != null) _updateme.cancel();
  }
              ...
              : Padding(
                  ...
                  child: PopupMenuButton(
                      ...
                      itemBuilder: (BuildContext context) => <PopupMenuEntry>[
                            const PopupMenuItem(
                                value: 'if-i-catch-u', child: Text('Logout')),
                            const PopupMenuItem(
                                value: 'i-don-catch-u',
                                child: Text('Settings')),
                          ]),
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: <BottomNavigationBarItem>[
          ...
          BottomNavigationBarItem(
              icon: Icon(Icons.schedule), title: Text('Notifications')),
          BottomNavigationBarItem(
              icon: Icon(Icons.view_list), title: Text('Transactions')),
          BottomNavigationBarItem(
              icon: Icon(Icons.credit_card), title: Text('Wallet')),
        ],
...
//      // print(value);
      ...
      setState(() {
        ...
        storage
            .setItem(STORAGE_USER_DETAILS_KEY, userDetails)
            .then<void>((value) {});
      });
    });
  }
  ...
  _refresh() {
    TabContent _productsScreen = TabContent(
      title: 'Home',
      content: ProductsScreen(
          currentUser: currentUser, userDetails: userDetails, reload: _initMe),
    );
    TabContent _notificationScreen = TabContent(
      title: 'Notifications',
      content: NotificationScreen(),
    );
    TabContent _transactionsScreen = TabContent(
      title: 'Transactions',
      content: TransactionsScreen(),
    );
    TabContent _walletScreen = TabContent(
      title: 'Wallet',
...
//    return _forAndroid();
    return WillPopScope(onWillPop: () => _exitApp(context), child: _foriOS());
  }
}
