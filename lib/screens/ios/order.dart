...
class _OrderProducts extends State<OrderProducts>{
  ...
  void _loadProducts(){
    void f(res){
      ...
      if(res is Map && res['data'] is List){
        setState(() {
          _orderProducts = res['data'].map<Map<String,dynamic>>((e)=>new Map<String, dynamic>.from(e)).toList();
        });
      }
    }
    _loading(true);
    ...
    if(_orderDetails.length>0){
      ...
      return Column(
        ...
        children: <Widget>[
          ...
          SizedBox(height: 10,),
          Expanded(
            child: ListView.builder(shrinkWrap: false,itemCount: _orderProducts.length,
            itemBuilder: (BuildContext ctxt, int index) {
