            ...
            child: Padding(
                ...
                child: TextField(
                  ...
                  onSubmitted: (e) => setState(() {
                    int qty = int.tryParse(e);
                    if (_schedule['product']['minimumQuantity'] > qty) {
                      Navigator.of(context).pop();
                      showDialog(
                          context: context,
                          child: SmartAlert(
                            title: 'Alert',
                            description:
                                "You have entered the minimum quantity allowed for this product.",
                          ));
                      return;
                    } else if (totalUnit > qty) {
                      Navigator.of(context).pop();
                      showDialog(
                          context: context,
                          child: SmartAlert(
                            title: 'Alert',
                            description:
                                'You can\'t choose quantity below it\'s current set unit',
                          ));
                      return;
                    } else if (qty > 9999) {
...
//        print(res);
        ...
        int _eventIdx =
            _events[item['time']].indexWhere((e) => e['id'] == item['id']);
        if (_eventIdx > -1) {
          setState(() {
                ...
                children: <Widget>[
                  ...
                  Row(
                    children: <Widget>[
                      ...
                      Expanded(
                          child: Text(
                              '${int.tryParse(_schedule['post']['quantity'])}  ${isNull(_schedule['product']['unit'], replace: 'unit')}',
                              style: TextStyle(color: noteColor))),
                      InkWell(
            ...
            Expanded(
                child: Card(
                    ...
                    child: Padding(
                      padding: EdgeInsets.all(5),
                      child: Text(
                          '${_schedule['product']['name']}, ${item['qty']} ${isNull(_schedule['product']['unit'], replace: 'unit')} $time (${item['title']})'),
                    ))),
            IconButton(
